...
export default function ProtocolLoadingScreen() {
  ...
  const protocolMessages = [
    ...
    "A otimizar a experiência de check-in...",
    "A verificar a sua permissão de acesso...",
    "A preparar o sistema de protocolo...",
  ];
  ...
  useEffect(() => {
    ...
    const messageInterval = setInterval(() => {
      setMessageIndex((prev) =>
        prev < protocolMessages.length - 1 ? prev + 1 : prev
      );
    }, 2500);
  ...
  const stats = [
    { label: "Convidados", icon: List },
    { label: "Check-ins", icon: UserCheck },
    { label: "QR Code", icon: QrCode },
    { label: "Acesso", icon: CalendarCheck },
  ];

  return (
    <div className="fixed inset-0 z-50 bg-[#f0f9ff] flex items-center justify-center text-[#374151] overflow-hidden">
      {/* Background Decoration */}
      <div className="absolute inset-0 bg-gradient-to-br from-[#f8faff] to-[#e0f2fe] animate-pulse-slow" />

      {/* Elegant Curtains */}
      {showCurtain && (
        <div className="absolute inset-0 flex z-30 pointer-events-none">
          <div className="w-1/2 h-full bg-[#dbeafe] animate-slide-left origin-left shadow-lg shadow-[#0000000a]" />
          <div className="w-1/2 h-full bg-[#dbeafe] animate-slide-right origin-right shadow-lg shadow-[#0000000a]" />
        </div>
      )}
      ...
      <UserCheck
        size={48}
        className="absolute bottom-8 right-8 text-green-500 opacity-70"
      />
      ...
      <div className="z-10 flex flex-col items-center animate-fade-in text-center px-6">
        ...
        <div className="relative w-16 h-16 mb-6">
          <motion.div
            className="absolute inset-0 rounded-full border-[3px] border-[#c4d0e0] border-t-green-500"
            initial={{ rotate: 0 }}
            animate={{ rotate: 360 }}
            transition={{
              duration: 1.5,
        ...
        {/* Brand/Event Name */}
        <div className="text-4xl sm:text-5xl font-extrabold bg-gradient-to-r from-blue-500 via-blue-600 to-teal-500 text-transparent bg-clip-text tracking-wide mb-6">
        Ateliê Badrú
        </div>

        {/* Protocol-specific "Status" display */}
